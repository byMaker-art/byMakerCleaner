...
final class SystemMonitor: ObservableObject {
    ...
    // Network
    @Published private(set) var networkBytesInDelta: Int64 = 0
    @Published private(set) var networkBytesOutDelta: Int64 = 0
    ...
    private func sampleNetwork() {
        ...
        while ptr != nil {
            ...
            // Only consider AF_LINK interfaces
            guard interface.ifa_addr.pointee.sa_family == UInt8(AF_LINK) else { continue }
            
            let name = String(cString: interface.ifa_name)
            // Skip loopback and awdl (Apple Wireless Direct Link used for AirDrop etc)
            if name.hasPrefix("lo") || name.hasPrefix("awdl") { continue }

            guard let ifaData = interface.ifa_data else { continue }
            let data = ifaData.assumingMemoryBound(to: if_data.self)
            currentBytesIn += UInt64(data.pointee.ifi_ibytes)
            currentBytesOut += UInt64(data.pointee.ifi_obytes)
        }

        if previousBytesIn > 0 && previousBytesOut > 0 {
            networkBytesInDelta = Int64(currentBytesIn > previousBytesIn ? currentBytesIn - previousBytesIn : 0)
            networkBytesOutDelta = Int64(currentBytesOut > previousBytesOut ? currentBytesOut - previousBytesOut : 0)
        }
