...
/// Detects whether Full Disk Access (FDA) has been granted to byMakerCleaner.
...
final class FullDiskAccessManager {
    ...
    /// the OS records byMakerCleaner as the requester and adds it to the FDA pane.
    ...
    /// Force byMakerCleaner to appear in the Full Disk Access list.
    ///
    ...
    /// *Finder* — not byMakerCleaner. So at launch we touch a small set of paths
    ...
    /// byMakerCleaner in unrelated permission lists ("Messages", "Contacts", etc.)
    ...
    /// Reveal the running byMakerCleaner.app bundle in Finder so the user can drag
    ...
    /// Reset byMakerCleaner's TCC entries so the OS can re-register the bundle.
    ...
    func resetFullDiskAccess() -> Bool {
        let bundleID = Bundle.main.bundleIdentifier ?? "com.bymakercleaner.app"
        let process = Process()
